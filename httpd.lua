...
do
	...
	local httpd_state = {}
	httpd_state.document_root = httpd.document_root or lfs.writedir()..[[Scripts\dcs-httpd\public_html\]]
	httpd_state.handlerdir = httpd.handlerdir or lfs.writedir()..[[Scripts\dcs-httpd\url_handlers\]]
	...
	local function reload_url_handlers()
		httpd.url_handlers = {
			["^/$"] = function(request, response)
					response:return_text("This is the default index page.\nhttpd version: "..httpd.version)
				end,
		}
		
		msg = ""
		for file in lfs.dir(httpd_state.handlerdir) do
			if lfs.attributes(httpd_state.handlerdir..file, "mode") == "file" and file:match(".lua$") then
				local bool, err = pcall(dofile, httpd_state.handlerdir..file)
				if bool then
					msg = msg .. "Successfully loaded: "..file.."\n"
				else
					msg = msg .. "Error executing "..file..": "..tostring(err).."\n"
				end
			end
		end
	...
	httpd_state.http_acceptor = socket.bind(httpd.bind_address or "localhost", 12080, 10)
	...
	local function http_handler(conn, conn_info)
		...
		response.set_status = function(response, status)
				...
				if status == 200 then response.status_text = "OK" end
				if status == 404 then response.status_text = "Not Found" end
				if status == 500 then response.status_text = "Internal Server Error" end
			end
		...
		if request.url == "/reload/" then
			handler_found = true
			local msg = reload_url_handlers()
			response:return_text("loaded new httpd handlers at mission time "..timer.getTime().."\n\n"..msg)
		end
		...
		if not handler_found then
			for url_prefix, url_handler in pairs(httpd.url_handlers) do
				if request.url:match(url_prefix) then
					...
					if not bool then
						response:return_text("failed to execute url handler for "..request.url..", error: "..tostring(err))
						response:set_status(500)
					end
					break
				end
		...
		if not handler_found then -- check if we want to server a static file instead
			if not request.url:gmatch("%.%.")() then -- prevent traversing parent directories
				local docroot = httpd_state.document_root
				local file = io.open(docroot..request.url, 'r')
				if not file then
					response:error_404()
					response.body = "File Not Found:"..docroot..request.url
				else
					response:set_status(200)
					response.body = file:read('*all')
					file:close()
					if request.url:match(".html$") then response.content_type = "text/html" end
					if request.url:match(".js$") then response.content_type = "application/javascript" end
				end
			end
		end
	...
	local function doComms(arg, time)
		...
		if http_newconn then
			...
			local newconn_info = { handler = coroutine.create(http_handler),
								   txbuf = "",
